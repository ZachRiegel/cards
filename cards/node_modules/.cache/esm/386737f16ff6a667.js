_6ff‍.x([["default",()=>_6ff‍.o]]);//these functions are mostly used in the construction of other lexical functions
let greed = (next) =>
    (current) =>
...
const lexical = new (class LexicalFunctions{
    ...
    tokenWasOfType = ({type}) => contextual((context) =>
        `${context.token.preferred} was a @${type}Token`
    );
    ...
    restoreHealth = ({quantity, isDynamic}) => contextual((context) =>
        `restore X health to ${context.target.preferred}`
    );
    ...
    cardName = (cardName) => capitalizeFirst(cardName.split(/(?=[A-Z])/).reduce((acc, val) => `${acc} ${capitalizeFirst(val)}`)).replace(/\\/g, '').replace(/_/g, ':').replace(/\|/g, ' /');
...
const interpret = (text) => {
    ...
    while(text.length) {
        ...
        currentInner = currentInner || {};
        if(typeof lexical[currentFunctionName] !== 'function') console.log('Function name:', currentFunctionName);
...
const interpretFunctionChain = (text) => {
    ...
    if(typeof lexical[functionName] !== 'function') console.log(functionName);
    return lexical[functionName](...args);
};

const _6ff‍anonymous=(s) => {
